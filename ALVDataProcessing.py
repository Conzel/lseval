...
class ALVData(object):
    ...
    def saveAKF(self, filepath):
        ...
        plt.clf()
        plt.semilogx(self.delay, self.akf, " bo", markersize = 2, label = \
                     "Autocorrelation data")
        popt, pcov = self.curveFit()
        plt.semilogx(self.delay, self.fitfun(self.delay, *popt), '-r', label = "exp. fit")  
        plt.legend()
...
def schoepeProcess(regString = r"DLS.*\.txt", samplereg = "_", path = os.getcwd(), \
                   fallOffFunction = (lambda x: 0.80*x), channelStart = 12):
    """
